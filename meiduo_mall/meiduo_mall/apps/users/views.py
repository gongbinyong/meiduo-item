...
class AccountsUserView(APIView):
    def get(self, request, username):
        ...
        try:
            ...
            redis_conn.setex('TOCKEN_%s'% token,300,user.mobile)
...
class AccountsPassowordView(APIView):
    def get(self, request,username):
        ...
        mobile = redis_conn.get('sms_code_%s'%sms_code)
        ...
        # redis_conn.setex('TOCKEN2_%s'% token,300,mobile)
...
class UndefinedPassowordView(APIView):
    # serializer_class = serializers.UndefinedPassowordlizer
    ...
    def post(self,request,user_id):
        ...
        if not access_token:
            return Response({'message': 'token不存在'}, status=status.HTTP_400_BAD_REQUEST)
        try:
        ...
        # mobile = redis_conn.get('TOCKEN2_%s' % access_token)
    ...
    def put(self,request,user_id):
        ...
        if not old_password:
            return Response({'message': '当前密码不能为空'}, status=status.HTTP_400_BAD_REQUEST)

        if not new_password:
            return Response({'message': '新密码不能为空'}, status=status.HTTP_400_BAD_REQUEST)

        if not new_password2:
            return Response({'message': '请确认新密码'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            user = User.objects.get(id=user_id)
        except User.DoesNotExist:
            return Response({'message': '用户不存在'}, status=status.HTTP_400_BAD_REQUEST)
