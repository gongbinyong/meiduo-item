...
class UserSerializer(serializers.ModelSerializer):
    ...
    def validate_allow(self, value):
        ...
        if value != 'true':
            raise serializers.ValidationError('请同意⽤用户协议')
        return value
    ...
    def validate(self, data):
        ...
        if data['password'] != data['password2']:
            raise serializers.ValidationError('两次密码不不⼀一致')
        # 判断短信验证码
...
class UndefinedPassowordlizer(serializers.ModelSerializer):
    ...
    def post(self,request):
        ...
        if not access_token:
            raise serializers.ValidationError('缺少token')
